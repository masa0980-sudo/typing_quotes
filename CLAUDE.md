...
**表示側**は同じファイルの`fetchCount()`(`playCounts/typing_quotes`をGETするだけの読み取り専用
関数)。`TitleScreen.tsx`がマウント時に1回呼び、結果が返るまでは何も表示しない
(ローディング表示はしない/取得失敗やドキュメント未作成時はnullのまま=非表示。
`AuthorAvatar`が肖像なし/読み込み失敗時にモノグラムへ黙って切り替えるのと同じ考え方)。
START ボタンの直下に「これまでに ◯ 回プレイされています」という一行で出し、
BEST SCORE 表示などと同じ小さめ・低コントラストのラベルスタイルに合わせている。
